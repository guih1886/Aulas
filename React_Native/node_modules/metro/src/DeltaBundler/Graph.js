...
class Graph {
  ...
  importBundleNames = new Set(); /// GC state for nodes in the graph (this.dependencies)
  ...
  async _processModule(path, delta, options) {
    const resolvedContext = this.#resolvedContexts.get(path); // Transform the file via the given option.
    ...
    this.dependencies.set(module.path, module); // Diff dependencies (1/2): remove dependencies that have changed or been removed.
  ...
  _incrementImportBundleReference(dependency) {
    var _this$gc$importBundle;

    const { absolutePath } = dependency;
    this.#gc.importBundleRefs.set(
      absolutePath,
      ((_this$gc$importBundle = this.#gc.importBundleRefs.get(absolutePath)) !==
        null && _this$gc$importBundle !== void 0
        ? _this$gc$importBundle
        : 0) + 1
    );
  ...
  _decrementImportBundleReference(dependency) {
    const { absolutePath } = dependency;
  ...
  _collectWhite(module, delta) {
    ...
    if (color === "white" && !this.#gc.possibleCycleRoots.has(module.path)) {
      ...
      for (const dependency of module.dependencies.values()) {
        const childModule = this.dependencies.get(dependency.absolutePath); // The child may already have been collected.
